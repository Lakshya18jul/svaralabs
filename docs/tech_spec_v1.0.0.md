...
| COD → Prepaid | Order created with COD gateway | 15 min after order | 1 retry, **next day's window** | **None** — every COD order qualifies | Confirm intent → pitch prepaid → **Shopify draft-order invoice link** via WhatsApp (optional discount) | Draft order paid |
